  ...
  do  
    ...
    do
      ...
      do
        ...
        if [ ${l4s_cc} = 'prague' ] ; then 
          ssh  ${IP_CLIENT_A} 'bash -s' < run_on_client.sh 3 prague ${IP_SERVER_A} &
          sleep 1
          ssh  ${IP_CLIENT_B} 'bash -s' < run_on_client.sh 0 cubic ${IP_SERVER_B} &
        elif [ ${l4s_cc} = 'dctcp' ]; then
          ssh  ${IP_CLIENT_A} 'bash -s' < run_on_client.sh 1 dctcp ${IP_SERVER_A} &
          sleep 1
          ssh  ${IP_CLIENT_B} 'bash -s' < run_on_client.sh 0 cubic ${IP_SERVER_B} &
        fi
